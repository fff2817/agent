...
def _search_docs_impl(
    ...
    *,
    ...
    logger.info(
        "[Tool] search_docs 检索: query=%r filenames=%s scope=%s",
        query,
        filenames,
        scope,
    )
...
def search_docs(
    query: str,
    filenames: list[str] | None = None,
    scope: Literal["auto", "all"] = "auto",
) -> str:
    """从已上传的文档知识库中检索相关内容。当用户询问文档、手册、PDF 中的信息，或需要基于资料回答时使用。系统会自动选择最相关的文件；若用户指定文件名，可传 filenames。返回最相关的文档片段（含来源和页码）。

    Args:
        query: 检索用的自然语言问题或关键词，例如「报销流程」
        filenames: 可选，限定检索的文件名列表，如 ["AI笔记.md"]
        scope: auto=自动路由选文件（默认）；all=搜索全部文档
    """
    return _search_docs_impl(query, filenames=filenames, scope=scope)
...
def run_search_docs(
    ...
    *,
    ...
    """兼容旧调用名。"""
    payload: dict = {"query": query, "scope": scope or "auto"}
    if filenames is not None:
        payload["filenames"] = filenames
    return search_docs.invoke(payload)
