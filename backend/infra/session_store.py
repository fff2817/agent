...
from lc.memory.types import ChatTurn, Session
...
class SessionStore:
    ...
    def _trim(self, session: Session) -> None:
        settings = get_settings()
        # 存储层尽量保留更多历史；Prompt 侧由 ConversationMemory 压缩
        max_turns = max(settings.session_store_max_turns, settings.max_session_turns)
        ...
        logger.info(
            "[Session] 截断存储历史: session_id=%s, 删除最早 %d 轮, 保留 %d 轮",
            session.session_id,
            removed,
            len(session.turns),
        )
    ...
    def clear_turns(self, session_id: str) -> None:
        ...
        session.updated_at = datetime.now(timezone.utc).isoformat()
        self._save(session)
        logger.info("[Session] 已清空对话: session_id=%s", session_id)
